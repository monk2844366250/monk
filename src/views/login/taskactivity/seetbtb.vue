...
export default {
  ...
  computed: {
    ...
    addpj: function() {
      ...
      if (this.form.commentType === 2) {
        ...
        if (length > count) {
          this.wzList.splice(this.form.orderNum, length - count)
        } else {
    ...
    //     var length = this.tpList.length
    //     if (length > count) {
    //       this.tpList.splice(this.form.orderNum, length - count)
    //     } else {
    //       for (var i = length; i < count; i++) {
    //         this.tpList.push({
    //           type: 3,
    ...
    //     var length = this.spList.length
    //     if (length > this.form.orderNum) {
    //       this.spList.splice(this.form.orderNum, length - count)
    //     } else {
  ...
  methods: {
    ...
    handleCommentType(val) {
      console.log('1447', val)
      // var length = this.form.orderNum
      console.log('判断是与否生效', this.form.orderNum)
      var length = 1
      if (val === 2) {
        this.form.taskCommentList = [{ type: 2, content: '' }]
        if (length < this.form.orderNum) {
          console.log('进来吗，没', this.form.orderNum)
          for (var i = length; i < this.form.orderNum; i++) {
            this.wzList.push({ type: 2, content: '' })
    ...
    handleNext() {
      ...
      data.taskCommentList = this.stitchingData(data.taskCommentList, otherKey, this.form.orderNum)
      this.form.storeId = this.store
      this.form.storeTypeId = this.storeType
      this.form.taskTypeId = this.taskType
