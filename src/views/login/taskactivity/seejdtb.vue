  ...
  <div style="margin:0 auto;width:1000px;">
    ...
    <div class="form">
      ...
      <div class="form-item">
        ...
        <el-input-number v-model="form.orderNum" size="mini" :min="1" style="width: 130px" @change="watchChange" />
      ...
      <div v-for="item in addpj" v-if="form.commentType===3">
        ...
        <div class="form-item">
          ...
          <el-input v-model="tpList[item-1].content" size="small" style="width: 350px" placeholder="" />
      ...
      <div v-for="item in addpj" v-if="form.commentType===4">
        ...
        <div class="form-item">
          <span class="form-required" />
          <span class="form-label">评语：</span>
          <el-input v-model="spList[item-1].content" size="small" style="width: 350px" placeholder="" />
      ...
      <div class="form-item">
        ...
        <el-input-number v-model="orderAll" size="mini" :min="1" style="width: 140px" />
        <span class="form-label" style="margin-left: 10px">单</span>
        <span style="color: #97A8BE;width: 100px;text-align: center">总共添加</span>
        <span>{{ inputAll }}</span>
      <!-- <span style="width:60px;" />
      <div>
        <span style="color: #97A8BE;width: 70px;text-align: center;font-size: 14px;display:inline-block;">放出</span>
        <span style="width: 100px" v-text="orderAll" />
      ...
      <div v-for="imgItem in imgList" class="form-item">
        ...
        <el-checkbox-group v-if="imgItem.name === '搜索货比'" v-model="screenshotDetails" style="display: flex;" @change="handleCheckBox">
          <div
          ...
          >
            ...
            <span v-if="childImgItem.amount>0" class="checkbox-tip">+{{ childImgItem.amount }}金/单</span>
          </div>
        </el-checkbox-group>
...
export default {
  ...
  computed: {
    ...
    addpj: function() {
      var count = 1
      count = this.form.orderNum
      console.log('weishenm,', count)
      if (this.form.commentType === 2) {
        var length = this.wzList.length
        if (length > count) {
          this.wzList.splice(count, length - count)
          console.log('商家123', this.form.taskCommentList)
        } else {
          for (var i = length; i < count; i++) {
            this.wzList.push({ type: 2, content: '' })
            console.log('商家', this.form.taskCommentList)
            console.log('商家', this.wzList)
          }
        }
  ...
  mounted() {
    ...
    this.form.taskTypeId = this.taskType // 12
  },
  methods: {
    watchChange() {
      console.log('设置')
      this.orderAll = this.form.orderNum
      this.inputAll = this.form.orderNum
    },
    // 改变时间内响应修改单数
    changeValue() {
    ...
    creatData(data) {
      ...
      reSetData({ id: data }).then(response => {
        if (response.code === 0) {
          ...
          // var reSetDatas = {}
          // reSetDatas = this.$store.getters.reSetData
          // console.log('我需要对比数据1', reSetDatas)
          ...
          this.form.guideEntrance = goodsdatas.postData.guideEntrance
          this.form.guideImgUrl1 = goodsdatas.postData.guideImgUrl1
          this.form.guideImgUrl2 = goodsdatas.postData.guideImgUrl2
          this.form.guideImgUrl3 = goodsdatas.postData.guideImgUrl3
          this.form.searchType = goodsdatas.postData.searchType
          this.form.orderNum = goodsdatas.postData.taskCommentList.length
          this.inputAll = goodsdatas.postData.taskCommentList.length
          this.orderAll = goodsdatas.postData.taskCommentList.length
    ...
    handleCommentType(val) {
      var length = 1
      if (val === 2) {
        this.form.taskCommentList = [{ type: 2, content: '' }]
        if (length < this.form.orderNum) {
          for (var i = length; i < this.form.orderNum; i++) {
            this.wzList.push({ type: 2, content: '' })
          }
        }
      } else if (val === 3) {
        this.form.taskCommentList = [{
          type: 3,
          content: '',
          norm: '',
          imgUrl1: '',
        ...
        if (length < this.form.orderNum) {
          for (var j = length; j < this.form.orderNum; j++) {
            this.tpList.push({
              type: 3,
              content: '',
              norm: '',
        ...
        console.log('this.form', this.form.taskCommentList)
      } else if (val === 4) {
        this.form.taskCommentList = [{ type: 4, content: '', norm: '', videoUrl: '' }]
        if (length < this.form.orderNum) {
          for (var m = length; m < this.form.orderNum; m++) {
            this.spList.push({ type: 4, content: '', norm: '', videoUrl: '' })
          }
        }
      } else if (val === 1) {
        this.form.taskCommentList = [{ type: 1 }]
        this.taskCommentVal = val
      }
      this.init()
    },
    ...
    handleNext() {
      ...
      if (this.form.commentType === 2) {
        ...
        otherKey = this.form.taskCommentList
      }
      ...
      // data.taskCommentList = this.stitchingData(this.form.taskCommentList, otherKey, this.form.orderNum)
      this.form.storeId = this.store
      this.form.storeTypeId = this.storeType
      this.form.taskTypeId = this.taskType
      ...
      data.taskCommentList = JSON.stringify(otherKey)
