...
<template>
  <div>
    ...
    <div class="form">
      ...
      <div v-for="(imgItem, index) in imgList" :key="index" class="form-item">
        <span class="form-required" />
      ...
      <div class="form-item">
        ...
        <el-checkbox-group v-model="form.taskAdditionalIncrement.goodsCategory">
          <div style="width: 550px;">
            <el-checkbox v-for="(item, index) in goodsCategoryList" :key="index" :label="item.id">{{ item.name }}</el-checkbox>
          </div>
        </el-checkbox-group>
      </div>
      <div class="form-item">
      ...
      <div v-if="expressFlag" class="form-item">
        ...
        <el-input v-model="form.taskAdditionalIncrement.weight" size="mini" type="number" style="width: 150px">
      ...
      <div class="form-item">
        ...
        <div style="position: relative">
          ...
          <i class="el-icon-circle-close avatar-uploader-remove" style="z-index:100;" @click="removeAddImg3()" />
        </div>
      </div>
      <div class="form-item">
        <span class="form-required" />
...
export default {
  ...
  computed: {
    ...
    addpj: function() {
      var count = 0
      // 当数据类型为1时，order1关键词，淘口令，二维码遍历，自加
  ...
  methods: {
    init() {
      var goodsdat = {}
      // var arrList = window.location.href.split('&tast=')
      // var arrtest = arrList[1].split('&')
      goodsdat = this.$route.query
      this.id = goodsdat.id
      this.store = goodsdat.postData.storeId
      this.storeType = goodsdat.postData.storeTypeId
      this.taskType = goodsdat.postData.taskTypeId
    ...
    creatData() {
      ...
      // console.log('再按那短刀卡', this.form.releaseDate)
      for (var k = 0; k < taskCommentListData.length; k++) {
        if (this.form.commentType === 1) {
          ...
          console.log('this.wzList[k]', this.wzList[k])
        } else if (this.form.commentType === 3) {
    ...
    // 删除
    removeAddVideo() {
      this.form.spList[0].videoUrl = ''
    },
    uploadImagePjtpChange(index, val) {
      this.pjtpIndex = index
      this.pjtpNum = val
    },
    uploadImagePjtpChangeVideo(index, val) {
      this.videoPjtpIndex = index
      this.videoPjtpNum = val
    },
    uploadImagePjtp(e) {
    ...
    handleNext() {
      ...
      // console.log('th..screenshotDetails', this.screenshotDetails)
      // console.log('判断data的数据', data.screenshotDetailIds.length, data.screenshotDetailIds)
      ...
      // console.log('data.taskAdditionalIncrement.goodsCategory', data.taskAdditionalIncrement.goodsCategory)
      ...
      // console.log('data.form', data.tastCommentList)
      // console.log('form.tast', this.form.tastCommentList)
      ...
      console.log('this.form.searchType', this.form.searchType)
      data.searchType = this.form.searchType
